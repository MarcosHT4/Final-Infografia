...
onready var goal = $EmeraldHillZone/Goal
onready var end_collision = $EmeraldHillZone/EndCollision/CollisionShape2D
...
func _process(delta):
	if not change_level and not audio_player.playing and (audio_player.stream == game_won_audio or audio_player.stream == game_over_audio):
		change_level = true
		if not return_to_level:
			LevelTitle.change_scene("res://scenes/Level2.tscn",2)
		else:
...
func create_ring():
	...
	ring_node.position.x = player.position.x - 1000
	ring_node.position.y = player.position.y - 500
	ring_node.scale.x = 2
	ring_node.scale.y = 2
